...
namespace ConsoleApp22
{
    class Program
    {
        ...
        private static async void Bot_OnMessage(object sender, MessageEventArgs e)
        {
            ...
            response1 = wc.DownloadString("https://webapplication120200526034526.azurewebsites.net/news");
            ...
            switch (message)
            {
               ...
               await client.SendTextMessageAsync(chatId: e.Message.Chat.Id, text: "Температура в Києві: " + response2 + " °C" + " \nВідчувається як: " + response3 + " °C" + " \n" + response4);                                   
                ...
                case "/searchnews":
                    {
                      await client.SendTextMessageAsync(chatId: e.Message.Chat, text: "Введіть заголовок новини, яку Ви хочете знайти:");
                ...
                case "/music":
                    {                       
                       await client.SendTextMessageAsync(chatId: e.Message.Chat, text: "Введіть назву пісні або виконавця:");
                        Messages.Add(message);
                        client.OnMessage += Bot_OnMessage;
        ...
        private static void GetWeather()
        {
            ...
            {
                response = webClient.DownloadString("https://webapplication120200526034526.azurewebsites.net/weather");
            }
        ...
        {
            WebClient wc = new WebClient();
            string res = wc.DownloadString($"https://webapplication120200526034526.azurewebsites.net/music?title={search}");
            if(res == "[]")
            {
                await client.SendTextMessageAsync(chatId: e.Message.Chat, "Нічого не знайдено, введіть команду /music знову, щоб знайти пісню");
            }
            ...
            foreach (JToken result2 in results)
            {
                ...
                await client.SendTextMessageAsync(chatId: e.Message.Chat, text: mr.name + "\n" + mr.url);
                
            }
            if (res != "[]")
                await client.SendTextMessageAsync(chatId: e.Message.Chat, "Якщо ви знову хочете знайти якусь пісню, введіть команду /music знову");

        }
        private async static void SearchNews(MessageEventArgs e, string search)
        {
            string news = GetStr($"https://webapplication120200526034526.azurewebsites.net/newssearch?title={e.Message.Text}");

            if (news != "")
            {
                await client.SendTextMessageAsync(e.Message.Chat.Id, news);
                await client.SendTextMessageAsync(chatId: e.Message.Chat, "Якщо ви знову хочете знайти якусь новину, введіть команду /searchnews знову");
            }
           else { await client.SendTextMessageAsync(e.Message.Chat, "Нічого не знайдено, введіть команду /searchnews знову, щоб знайти іншу новину");  }
